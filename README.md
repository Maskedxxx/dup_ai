...
### 4. Интеллектуальная фильтрация (Tools) (`app/tools/`)
Для более точного и гибкого поиска данных (например, рисков внутри большого проекта) была внедрена архитектура инструментов (Tools). Вместо простой фильтрации по названию проекта, система теперь использует LLM для выбора и выполнения наиболее подходящего инструмента для анализа запроса.

- **ToolRegistry** - Центральный реестр, который автоматически находит и регистрирует все доступные в системе инструменты.
- **ToolExecutor** - Компонент, который получает запрос, обращается к LLM с просьбой выбрать инструмент и сформировать для него аргументы, а затем выполняет выбранный инструмент.
- **BaseTool** - Абстрактный класс, определяющий контракт для всех инструментов (методы `get_schema` и `execute`).
- **KeywordSearchTool** - Реализация инструмента для интеллектуального поиска по ключевым словам с использованием лемматизации для понимания различных форм слов.

#### Стратегии фильтрации
Система использует конфигурируемые стратегии фильтрации для разных типов данных:
- **none** - без дополнительной фильтрации (подрядчики, ошибки, процессы)
- **keybert** - интеллектуальный поиск по ключевым словам (риски)
...
6. **[ШАГ 4] Загрузка элементов** для классификации (например, названий проектов)
7. **[ШАГ 5] Классификация** - определение релевантного элемента (проекта) через LLM
8. **[ШАГ 6] Интеллектуальная фильтрация** - LLM выбирает и исполняет инструмент (`ToolExecutor`) для поиска релевантных записей (например, рисков) по смыслу запроса внутри найденного проекта.
9. **[ШАГ 7] Преобразование** - создание доменных моделей из отфильтрованных данных
