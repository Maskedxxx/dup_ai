...
# Карта соответствия ID категорий и их внутренних значений
CATEGORY_MAPPING = {
    RiskCategory.NIOKR: "niokr",
    RiskCategory.PRODUCT_PROJECT: "product_project",
    RiskCategory.MANUFACTURING: "manufacturing"
}
...
class RiskClassifierService:
    """
    Сервис для классификации запросов о рисках и определения релевантных проектов.
    """
    ...
    def __init__(self, llm_client: LLMClient):
        ...
        logger.info("Инициализирован сервис классификации рисков")
    ...
    def load_project_names(self, df: pd.DataFrame) -> List[str]:
        ...
        """
        try:
            # Получаем уникальные значения из колонки 'project_name'
            if 'project_name' in df.columns:
                # Фильтруем пустые значения и преобразуем в список
                unique_names = df['project_name'].dropna().unique().tolist()
                ...
                logger.warning("Колонка 'project_name' не найдена в данных")
                return []
    ...
    def classify(self, question: str) -> str:
        ...
        """
        logger.info(f"Классификация запроса о рисках: '{question}'")
        ...
        prompts = PromptBuilder.build_risk_project_classification_prompt(question, self.project_names)
        ...
        try:
            ...
            logger.error(f"Ошибка при классификации запроса о рисках: {e}")
            return ""
    
    def filter_risks(self, df: pd.DataFrame, project_name: str) -> Tuple[pd.DataFrame, Dict[int, float]]:
        """
        ...
        """
        logger.info(f"Фильтрация рисков по названию проекта: '{project_name}'")
        
        if not project_name:
            logger.warning("Не указано название проекта для фильтрации, возвращаем все данные")
            return df.copy(), {}
        
        # Проверяем наличие колонки 'project_name'
        if 'project_name' not in df.columns:
            logger.warning("Колонка 'project_name' не найдена в данных")
            return df.copy(), {}
        
        filtered_df = df[df['project_name'] == project_name].copy()
        
        # Если не найдено рисков для данного проекта, возвращаем пустой DataFrame
        if len(filtered_df) == 0:
            logger.warning(f"Не найдено рисков для проекта '{project_name}'")
            return pd.DataFrame(), {}
        
        # Создаем словарь с оценками релевантности (все строки с равной оценкой)
        scores = {idx: 1.0 for idx in filtered_df.index}
        
        logger.info(f"Найдено {len(filtered_df)} рисков для проекта '{project_name}'")
